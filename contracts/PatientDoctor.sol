...
contract PatientRegistry {
    ...
    function _contains(bytes memory data, bytes memory needle) private pure returns (bool) {
        if (needle.length == 0 || needle.length > data.length) return false;
    ...
    // Medical records stored as JSON wrapper
    function _isLikelyMedicalWrapper(string memory encryptedData) private pure returns (bool) {
        bytes memory payload = bytes(encryptedData);
        return
            _contains(payload, bytes('"doctor"')) &&
            _contains(payload, bytes('"patient"')) &&
            _contains(payload, bytes('"ciphertext"')) &&
            _contains(payload, bytes('"ephemPublicKey"')) &&
            _contains(payload, bytes('"nonce"')) &&
            _contains(payload, bytes('"version"'));
    }
    ...
    function completeAppointmentAndAddRecordByDoctor(
        ...
        require(_isLikelyMedicalWrapper(encryptedData), "Required medical payload");
